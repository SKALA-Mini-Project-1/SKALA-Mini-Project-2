...
- 현재 `QueuePriorityPolicy`는 중립값 `0L`을 반환한다.
- 즉, 지금 대기열은 팬 점수 가중치 없이 기본 순서로 동작한다.
...
- [ ] 팬 점수 데이터의 최종 소스 결정
- [ ] 내부 API 호출 기반으로 점수 조회할지 여부 결정
- [ ] 아티스트 기준 점수 계산 규칙 명문화
- [ ] 점수 상한선과 음수/이상치 처리 규칙 확정
- [ ] 캐시 사용 여부와 TTL 정의
- [ ] downstream 장애 시 우선순위 fallback 정책 정의
- [ ] `QueueService.startTicketing()` 기준 우선순위 테스트 추가
...
- 현재 대기열 기본 흐름, 스케줄러 점검, Redis fallback 테스트가 먼저 정리되어 팬 점수 정책은 그 위에 안전하게 연결할 수 있는 상태를 만드는 중이다.
